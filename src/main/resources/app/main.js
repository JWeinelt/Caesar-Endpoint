...
function displayPluginMostDownloads(pdata) {
    ...
    let el = `
                <div class="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                    <img src="http://${ADDRESS}/api/image/${pdata.screenshots[0]}?type=screenshot" alt="${pluginName}" class="w-full h-48 object-cover">
                    <div class="p-4">
                        <h3 class="text-xl font-semibold"><a href="/view?plugin=${pluginName}">${pluginName}</a></h3>
                        <p class="text-sm text-gray-400 mb-4">${pdata.description}</p>
                        <p class="text-m font-semibold">by <a class="devname" href="#">${pdata.author}</a></p>
